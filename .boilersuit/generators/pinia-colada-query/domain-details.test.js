...
import { {{ QUERY_KEY }}_KEYS, use{{ COMPOSABLE_NAME }}Details } from ".";

/**
 * Create the {{ NAME | kebab }} details query wrapper in a Vue app context.
 *
 * @param  {string|null}  {{ ID_NAME }}
 *     The item ID to pass to the query wrapper.
 */
function create{{ COMPOSABLE_NAME }}Details({{ ID_NAME }} = "item-123") {
	return withAppContext(() => use{{ COMPOSABLE_NAME }}Details({{ ID_NAME }}));
}

describe("{{ NAME | kebab }} details", () => {
	setupConsole();
	...
	describe("{{ QUERY_KEY }}_KEYS", () => {
		test("Creates a stable record key", () => {
			expect({{ QUERY_KEY }}_KEYS.byId("item-123")).toEqual(["{{ NAME | kebab }}", "item-123"]);
		});
	});

	describe("use{{ COMPOSABLE_NAME }}Details", () => {
		test("Initialises with no {{ NAME | kebab }} details", () => {
			const {
				...
				refetch,
				{{ DATA_NAME }},
			} = create{{ COMPOSABLE_NAME }}Details(null);
