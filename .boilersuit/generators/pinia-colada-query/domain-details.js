...
export const {{ QUERY_KEY }}_QUERY_KEY = ({{ ID_NAME }}) => {{ QUERY_KEY }}_KEYS.byId({{ ID_NAME }});
...
export function use{{ COMPOSABLE_NAME }}({{ ID_NAME }}) {
	const current{{ COMPOSABLE_NAME }} = useQueryWrapper({
		queryOptions: () => ({
			...{{ DATA_NAME }}QueryOptions(unref({{ ID_NAME }})),
...
async function get{{ COMPOSABLE_NAME }}({{ ID_NAME }}) {
	return shape{{ COMPOSABLE_NAME }}Response(await get(`{{ ENDPOINT }}/${{{ ID_NAME }}}`));
}
