...
// Stand-in for modal-dialog that renders its title and default slots and can
// emit dialog:close, so tests can drive the close path without the real dialog.
...
// Mount helper with the modal dialog and form wrapper stubbed, so tests exercise
// the form's own mode, state, and submit logic rather than their rendering.
...
describe("{{ SINGULAR_NAME | kebab }} modal form", () => {
	...
	describe("Dialog", () => {
		test("Emits close when the dialog closes", () => {
			const wrapper = mountForm();

			wrapper.getComponent(modalDialogStub).vm.$emit("dialog:close");
	...
	describe("Submission", () => {
		...
		test("Emits the successful result, submitted form data, and close", async () => {
			const values = { name: "Example" };
