<template>
	<modal-dialog focus-dialog-on-open @dialog:close="emit('close')">
		<template #title>
			<!-- TODO: Replace with project-specific item copy. modal-dialog wraps this slot in its own heading, so keep it to text. -->
			<span v-text="isEditMode ? 'Edit item' : 'Add item'" />
		</template>
...
/**
 * close is emitted when the dialog asks to close (its `dialog:close` event) and
 * again after a successful submit, so the parent can unmount the modal. This
 * form makes no routing decisions of its own.
 *
